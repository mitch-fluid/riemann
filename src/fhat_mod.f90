...
contains 
    subroutine fhat(ql, qr, f, ib, ie, jb, je)  
        ! arguments 
        ...
        do j = jb, je 
            do i = ib, ie 
                ...
                ! compute flux 
                f(i,j,1) = 0.5*(ql(i,j,2) + qr(i,j,2) - diss(1))
                f(i,j,2) = 0.5*(ql(i,j,2)*ul + pl + qr(i,j,2)*ur + pr - diss(2))
                f(i,j,3) = 0.5*(ql(i,j,2)*Hl + qr(i,j,2)*Hr - diss(3))
                f(i,j,4) = 0.5*(ql(i,j,2)*vl + qr(i,j,2)*vr - diss(4)) 
            end do 
        end do 
